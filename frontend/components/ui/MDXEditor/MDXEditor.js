...
export default function MDXEditor({ 
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const imageData = await onImageUpload(file, articleId);
  ...
  const uploadImageCommand = {
    ...
    icon: isUploading ? (
      <div style={{width: '12px', height: '12px', border: '2px solid #ccc', borderTop: '2px solid #007acc', borderRadius: '50%', animation: 'spin 1s linear infinite'}}></div>
    ) : (
  ...
  const previewCommands = [
    ...
    },
    {
      ...
      icon: (
        <svg width="12" height="12" viewBox="0 0 20 20" fill="currentColor">
          <path d="M2 4a2 2 0 012-2h12a2 2 0 012 2v12a2 2 0 01-2 2H4a2 2 0 01-2-2V4zm4 0v12h8V4H6z"/>
        </svg>
      ),
